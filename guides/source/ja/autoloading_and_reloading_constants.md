...
この設定の目的は、`AUTH_SERVICE`で示される環境に対応するクラスをRailsアプリケーションから使えるようにすることです。developmentモードでは、イニシャライザ実行時に`MockedAuthService`が自動読み込みされます。ここで、アプリケーションでいくつかのリクエストを処理した後、実装に変更を加え、再度アプリケーションにアクセスしたとしましょう。
...
さらにアプリケーションで`AuthService.instance`を代りに使うという方法があります。`AuthService`は必要に応じて読み込まれ、自動読み込みとよく調和します。
