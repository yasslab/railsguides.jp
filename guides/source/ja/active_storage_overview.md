...
開発およびテスト用のローカルディスクベースのサービスが付属しており、ファイルをバックアップおよび移行用の従属サービスにミラーリングできます。

Active Storageを使うと、アプリケーションは[ImageMagick](https://www.imagemagick.org)で画像のアップロードを変換し、
PDFやビデオなどの非画像アップロードの画像表現を生成し、任意のファイルからメタデータを抽出できます。

## セットアップ

Active Storageは、アプリケーションのデータベースで `active_storage_blobs`と`active_storage_attachments`という名前の2つのテーブルを使います。
...
Active Storageのサービスを`config/storage.yml`で宣言してください。 アプリケーションが使うサービスごとに、名前と必要な構成を指定します。 
...
それぞれの環境では異なるサービスが使われる可能性が高いため、これを環境ごとに行うことをお勧めします。 開発環境の前の例のディスクサービスを使うには、`config/environments/development.rb`に以下を追加します。
...
ミラー化されたすべてのサービスで実行されます。ミラーリングされたサービスを使って、プロダクション内のサービス間の移行を容易にすることができます。 新しいサービスへのミラーリングを開始したり、既存のファイルを古いサービスから新しいものにコピーしたり、新しいサービスにオールインすることができます。
上記のように、利用する各サービスを定義し、ミラー化されたサービスから参照します。
...
Active Jobを使うようにアプリケーションが設定されている場合は、バックグラウンドで削除を実行できます。消去すると、BLOBとファイルがストレージサービスから削除されます。
...
ダウンロードリンクを作成するには、`rails_blob_ {path | url}`ヘルパーをお使いください。このヘルパーを用いて処理を設定できます。
...
これらのライブラリはRailsでは提供されていません。組み込みのプレビューアを使うには、それらを自分でインストールする必要があります。
サードパーティのソフトウェアをインストールして使う前に、ライセンスの影響を理解していることを確認してください。


サービスに直接アップロードする
--------------------------

Active Storageは、付属のJavaScriptライブラリを用いて、クライアントからクラウドへの直接アップロードをサポートします。
...
これらのイベントを使って、アップロードの進行状況を表示できます。
...
システムが添付ファイルを含むモデルの削除を検証し、Active Jobを使っている場合は、インラインキューアダプタを使うようにテスト環境を設定して、未知の時間ではなく即時にパージジョブを実行します。

また、テスト環境で別のサービス定義を使って、開発中に作成したファイルをテストで削除しないようにすることもできます。
