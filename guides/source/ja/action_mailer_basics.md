...
Action Mailerを使うと、アプリケーションのメイラークラスやビューでメールを送信することができます。

#### メイラーはコントローラと似ている

メイラーは`ActionMailer::Base`を継承し、`app/mailers`に配置されます。また、メイラーの動作はコントローラともきわめて似通っています。似ている点の例として、メイラーにあるものを以下にリストアップしてみました。

* アクションがあり、`app/views`下のビューにも関連付けられている
* インスタンス変数を経由してビューにアクセスできる
* レイアウトとパーシャルを利用できる
* paramsハッシュにアクセスできる
...
ジェネレータを使いたくない場合は、`app/mailers`ディレクトリ以下にファイルを作成し、`ActionMailer::Base`を継承するだけで作業できます。
...
NOTE: `deliver_later`を呼ぶと、そのジョブは`mailers`キューの下に配置されます。Active Jobアダプタがこのジョブをサポートするようにしておいてください。そうしないと、ジョブが警告なしに無視されてメールを送信できなくなります。この動作は`config.action_mailer.deliver_later_queue_name`を指定することで変更できます。

メールをcronjobなどから今すぐ送信したい場合は、`deliver_now`を呼び出すだけで送信できます。
...
デフォルトの`app/views/mailer_name/`ディレクトリの外にあるテンプレートでレンダリングしたい場合は、以下のように`prepend_view_path`を適用する方法が使えます。

```ruby
class UserMailer < ApplicationMailer
  prepend_view_path "custom/path/to/mailer/view"

  # "custom/path/to/mailer/view/welcome_email"テンプレートを読み込もうとする
  def welcome_email
    # ...
  end
end
...
または[`append_view_path`](https://guides.rubyonrails.org/action_view_overview.html#ビューのパス)メソッドを検討してもよいでしょう。
...
Action Mailerは、メールのオブザーバー（observer）やインターセプタ（intercepter）のメソッドへのフックを提供しています。これを用いることで、メール送信のたびに配信ライフサイクル期間中に呼び出されるクラスを登録できます。

### メールをインターセプトする

インターセプタを用いることで、メールを配信エージェントに引き渡す前にメールを加工できます。インターセプタのクラスは`:delivering_email(message)`メソッドを実装することが必須条件です。このメソッドはメール送信前に呼び出されます。
...
インターセプタが動作するようにするには、Action Mailerフレームワークに登録しておく必要があります。これは、以下のようにイニシャライザファイル`config/initializers/sandbox_email_interceptor.rb`で行います。

```ruby
if Rails.env.staging?
  ActionMailer::Base.register_interceptor(SandboxEmailInterceptor)
end
```nMailer::Base.register_interceptor(SandboxEmailInterceptor) if Rails.env.staging?
```
...
オブザーバーを用いることで、送信された後のメールメッセージにアクセスできるようになります。オブザーバーのクラスは`:delivered_email(message)`メソッドを実装することが必須条件です。このメソッドはメール送信後に呼び出されます。
...
インターセプタの場合と同様に、オブザーバーもAction Mailerフレームワークに登録しておく必要があります。これは、以下のようにイニシャライザファイル`config/initializers/email_delivery_observer.rb`で行います。
